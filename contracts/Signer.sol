 ...
 * @title Signer
 */
contract Signer is Ownable, ERC20PresetMinterPauser {
    ...
    constructor (
        ...
        uint8 decimals
    ) ERC20PresetMinterPauser(name, symbol) payable {
    ...
    function burnFrom(address _from, uint256 amount) public override onlyOwner() {
        _burn(_from, amount);
    ...
    function grantMinterRoleToAddress(address _address) external onlyOwner(){
        _setupRole(MINTER_ROLE, _address);
    }

    function grantAdminRoleToAddress(address _address) external onlyOwner(){
